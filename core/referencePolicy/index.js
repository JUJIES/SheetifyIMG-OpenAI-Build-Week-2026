...
function inferReferencePolicy(input = {}) {
  ...
  if (hasQr) {
    return policy({
      ...
      preferredSource: "app_template",
      suggestedAction: "QR-Code deterministisch erzeugen und als festes Asset einbetten.",
  ...
  if (hasCoordinateSystem) {
    return policy({
      ...
      preferredSource: "app_template",
      suggestedAction: "Koordinatensystem als App-Vorlage oder hochgeladene Referenz nutzen.",
      allowImageModelToRedraw: false,
  ...
  if (hasExactLayout) {
    return policy({
      ...
      preferredSource: "app_template_or_user_upload",
      suggestedAction: "Geeignete Vorlage oder Referenzbild anhängen.",
      allowImageModelToRedraw: false,
      canProceedWithoutReference: true,
  ...
  if (hasLocalVisualReference) {
    return policy({
      ...
      label: "Webreferenz sinnvoll",
      reason: "Lokale visuelle Details wie Schilder, Straßenecken oder stadttypische Gestaltung wirken ohne Referenz schnell generisch. Eine Webreferenz stabilisiert Form, Material, Farbe und Kontext.",
      triggers: ["lokaler visueller Kontext"],
      preferredSource: "user_upload_or_reference_search",
      suggestedSearchQuery: "Berlin Straßenschild Wikimedia Commons",
      suggestedAction: "Passende offen lizenzierte Webreferenz suchen oder eigenes Referenzbild hochladen.",
