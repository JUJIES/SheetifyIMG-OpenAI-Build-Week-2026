...
const TOOL_DESCRIPTIONS = Object.freeze({
  ...
  approve_current_content: "Schlaegt vor, das aktuelle Arbeitsblatt-Konzept fuer Entwürfe freizugeben.",
  ...
  create_run: "Schlaegt vor, aus dem freigegebenen Arbeitsblatt-Konzept eine Entwurfsrunde anzulegen.",
  ...
  generate_candidate_from_content_proposal: "Alter Kombi-Schritt. Im normalen Workflow nicht anbieten: erst Konzept uebernehmen oder aktualisieren, danach Entwurf erstellen.",
  ...
  adopt_image_spec: "Schlaegt vor, die Entwurfsvorbereitung fuer die Bildgenerierung zu uebernehmen.",
  prepare_reference_asset: "Schlaegt vor, eine benoetigte Referenz oder App-Vorlage fuer die naechste Entwurfserstellung vorzubereiten.",
  prepare_web_reference_asset: "Schlaegt vor, eine offen lizenzierte Webreferenz zu suchen, herunterzuladen und als Bildreferenz fuer die naechste Entwurfserstellung zu verwenden.",
  generate_image_candidate: "Schlaegt vor, nach Konzept-Freigabe einen Entwurf zu erstellen. Die App leitet die ImageSpec intern ab und die UI muss bezahlte Generierung bestaetigen. Niemals behaupten, dass keine weitere Bestaetigung noetig ist.",
...
function buildAiTools(workspace) {
  const commands = visibleWorkflowCommands(workspace)
    .filter(isEnabledWorkflowCommand)
    .slice(0, 2);
...
function suggestedActionsFromToolCalls(toolCalls, workspace) {
  const commandsById = new Map(visibleWorkflowCommands(workspace).map((command) => [command.id, command]));
  const actions = [];
  const seen = new Set();
