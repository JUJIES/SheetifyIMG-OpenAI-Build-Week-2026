...
function cleanLabel(value, fallback = "SheetifyIMG Pass") {
...
function passRecord(state, passId) {
  ...
  if (!pass) {
    throw Object.assign(new Error("SheetifyIMG Pass wurde nicht gefunden."), { statusCode: 404 });
  }
  return pass;
}
...
function assertPassActive(pass, now = new Date().toISOString()) {
  if (!ACTIVE_PASS_STATUSES.has(pass.status)) {
    throw Object.assign(new Error("Dieser SheetifyIMG Pass ist derzeit nicht aktiv."), { statusCode: 403 });
  }
  if (pass.expiresAt && pass.expiresAt <= now) {
    throw Object.assign(new Error("Dieser SheetifyIMG Pass ist abgelaufen."), { statusCode: 403 });
  }
}
...
function createBetaAccessManager(config = {}) {
  ...
  async function createRecoveryChallenge(requestId, options = {}) {
    ...
    return transact((state, now) => {
      ...
      if (!request.passId) {
        throw Object.assign(new Error("Zu dieser E-Mail ist kein SheetifyIMG Pass hinterlegt."), { statusCode: 409 });
      }
  ...
  async function loginWithPass(code, deviceName, options = {}) {
    ...
    return transact((state, now) => {
      ...
      if (!pass) {
        throw Object.assign(new Error("Der SheetifyIMG Pass ist ungültig."), { statusCode: 401 });
      }
      assertPassActive(pass, now);
      const created = createSessionRecord(pass, deviceName, now, options.uiLocale);
      state.sessions.push(created.session);
